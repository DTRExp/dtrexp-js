...
> [!IMPORTANT]
> Parse **once** (at write/config time), evaluate **many** — `DTRE` instances are immutable, and `covers()` performs a single calendar-field extraction followed by integer comparisons. No occurrence iteration, ever.
...
| --- | --- |
| `parse(expression)` | Parses a DTRE string into an immutable `DTRE`. Throws `DTRESyntaxError` with a stable `code` and character `position` on invalid input. The only way to construct a `DTRE`. |
...
The full grammar and semantics live in the **[specification](https://github.com/DTRExp/dtre-spec/blob/main/draft-2.md)**; the essentials:
