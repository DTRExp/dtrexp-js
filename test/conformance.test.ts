...
// test/vectors.json is vendored verbatim from DTRExp/dtre-spec — do not edit here.
...
describe('conformance: covers()', () => {
  for (const group of vectors.coverage) {
    describe(`${group.id} — '${group.expression}' [${group.tz}]`, () => {
      const dtre = parse(group.expression);
      for (const [instant, expected] of Object.entries(group.cases)) {
        it(`${instant} → ${expected}`, () => {
          expect(dtre.covers(instant, { tz: group.tz })).toBe(expected);
        });
      }
    });
  }
